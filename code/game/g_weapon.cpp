...
static	vec3_t	wpForward, wpVright, wpUp;//forward, vright, up;
...
//	VectorCopy( self->currentOrigin, self->s.pos.trBase );
	AngleVectors( self->s.angles, wpForward, NULL, NULL );

	if ( self->fxID > 0 )
	{
		G_PlayEffect( self->fxID, self->currentOrigin, wpForward );
	}
...
//---------------------------------------------------------
{
	...
	G_PlayEffect( G_EffectIndex( "disruptor/line_cap" ), muzzle, wpForward );
}
...
//---------------------------------------------------------
{
	...
	CalcMuzzlePoint( self, wpForward, wpVright, wpUp, muzzle, 0 );
	VectorNormalize( wpForward );
	VectorMA( muzzle, -4, wpForward, muzzle );
...
//---------------------------------------------------------
{
	...
	{
		WP_DropDetPack( ent, muzzle, wpForward );
...
//---------------------------------------------------------
{
	gentity_t	*missile = CreateMissile( muzzle, wpForward, BRYAR_PISTOL_VEL, 10000, ent );
...
//---------------------------------------------------------
{
	...
	gentity_t	*missile = CreateMissile( muzzle, wpForward, vel, 10000, ent );
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( muzzle, wpForward, vel, 10000, ent, qtrue );
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( muzzle, wpForward, ATST_SIDE_MAIN_VELOCITY, 10000, ent, qfalse );
...
//---------------------------------------------------------
{
	...
	VectorMA( muzzle, range, wpForward, end );
...
//---------------------------------------------------------
void CalcMuzzlePoint( gentity_t *const ent, vec3_t wpForward, vec3_t right, vec3_t up, vec3_t muzzlePoint, float lead_in ) 
//---------------------------------------------------------
{
	vec3_t		org;
	...
	case WP_BRYAR_PISTOL:
		...
		muzzlePoint[2] -= 16;
		VectorMA( muzzlePoint, 28, wpForward, muzzlePoint );
	...
	case WP_BLASTER:
		...
		muzzlePoint[2] -= 1;
		if ( ent->s.number == 0 )
			VectorMA( muzzlePoint, 12, wpForward, muzzlePoint ); // player, don't set this any lower otherwise the projectile will impact immediately when your back is to a wall
		else
			VectorMA( muzzlePoint, 2, wpForward, muzzlePoint ); // NPC, don't set too far forward otherwise the projectile can go through doors
	...
	case WP_SABER:
		...
		VectorMA( muzzlePoint, 8, wpForward, muzzlePoint );
...
//---------------------------------------------------------
{
	...
	CalcMuzzlePoint ( ent, wpForward, wpVright, wpUp, muzzle , 0);
